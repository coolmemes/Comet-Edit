    ...
    public void handle(Session client, MessageEvent msg) {
        ...
        if (data == null) {
            if (NetworkManager.getInstance().getSessions().getByPlayerId(userId) != null) {
                ...
                hideOnline = NetworkManager.getInstance().getSessions().getByPlayerId(userId).getPlayer().getSettings().getHideOnline();
            }
        }
        ...
        if (data == null) {
            ...
            hideOnline = PlayerDao.getSettingsById(userId).getHideOnline();
        }
        ...
        client.send(new LoadProfileMessageComposer(data, stats, groups, client.getPlayer().getMessenger().getFriendById(userId) != null, false, hideOnline, BanManager.getInstance().hasBan(Integer.toString(data.getId()), BanType.USER)));
    }
}
