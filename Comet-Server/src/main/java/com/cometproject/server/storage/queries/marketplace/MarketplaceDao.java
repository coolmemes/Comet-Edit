package com.cometproject.server.storage.queries.marketplace;
...
public class MarketplaceDao {
    public static void loadOffers(List<MarketplaceOfferItem> offers) {
        ...
        try {
            try {
                ...
                preparedStatement = SqlHelper.prepare("SELECT * FROM catalog_marketplace_offers", sqlConnection);
    ...
    public static MarketplaceOfferItem createOffer(int playerId, PlayerItem playerItem, int price, int finalPrice, int type) {
        ...
        try {
            sqlConnection = SqlHelper.getConnection();
            preparedStatement = SqlHelper.prepare("INSERT INTO catalog_marketplace_offers VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?)", sqlConnection, (boolean) true);
    ...
    public static void updateOffer(MarketplaceOfferItem offer) {
        ...
        try {
            try {
                sqlConnection = SqlHelper.getConnection();
                preparedStatement = SqlHelper.prepare("UPDATE catalog_marketplace_offers SET state = ? WHERE id = ? LIMIT 1", sqlConnection);
    ...
    public static int getItemIdByOffer(int offerId) {
        ...
        try {
            sqlConnection = SqlHelper.getConnection();
            preparedStatement = SqlHelper.prepare("SELECT item_id FROM catalog_marketplace_offers WHERE id = ? LIMIT 1", sqlConnection);
    ...
    public static void deleteOffer(MarketplaceOfferItem offer) {
        ...
        try {
            try {
                sqlConnection = SqlHelper.getConnection();
                preparedStatement = SqlHelper.prepare("DELETE FROM catalog_marketplace_offers WHERE id = ? AND player_id = ? LIMIT 1", sqlConnection);
