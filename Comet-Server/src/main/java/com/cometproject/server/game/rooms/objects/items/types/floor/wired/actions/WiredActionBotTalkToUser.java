...
public class WiredActionBotTalkToUser extends WiredActionItem {
     ...
     */
    public WiredActionBotTalkToUser(long id, int itemId, Room room, int owner, int x, int y, double z, int rotation, String data) {
    ...
    public void onTickComplete() {
        if (this.botEntity != null) {
            ...
            if (isWhisper) {
                playerEntity.getPlayer().getSession().send(new WhisperMessageComposer(botEntity.getId(), message, 2));
            } else {
                playerEntity.getPlayer().getSession().send(new TalkMessageComposer(botEntity.getId(),  playerEntity.getUsername() + " " + message, ChatEmotion.NONE, 2));
            }
        }
