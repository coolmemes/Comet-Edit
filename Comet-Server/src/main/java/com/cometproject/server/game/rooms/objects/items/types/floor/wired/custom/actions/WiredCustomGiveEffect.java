 ...
 */
public class WiredCustomGiveEffect extends WiredActionItem {
    /**
     ...
     */
    public WiredCustomGiveEffect(long id, int itemId, Room room, int owner, int x, int y, double z, int rotation, String data) {
    ...
    public void onTickComplete() {
        ...
        int effectId = Integer.parseInt(this.getWiredData().getText());
        ...
        if(minimumRank != null && playerEntity.getPlayer().getData().getRank() < minimumRank) {
            playerEntity.getPlayer().getSession().send(new WhisperMessageComposer(playerEntity.getId(), Locale.get("wired.minimum_rank.error"), 34));
            return;
        }
